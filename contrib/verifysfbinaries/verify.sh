...
###   This script attempts to download the signature file SHA256SUMS.asc from javabit.org
...
WORKINGDIR="/tmp/javabit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://javabit.org/bin/"
VERSIONPREFIX="javabit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'javabit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Javabit Core binary release signing key installed?"
   fi
